...
if (isset($_GET['show'])) {
  ...
  $sql = "SELECT `order_id` FROM `food_items` WHERE `food_items`.`sno` = $lockid; ";
...
<div class="container my-4">
  <table class="table" id="myTable">
    ...
    <tbody>
      <?php
      $sql = "SELECT * FROM `food_items` WHERE `order_id` = '$lockid';";
      $result = mysqli_query($conn, $sql);
  ...
  Array.from(deletes).forEach((element) => {
    element.addEventListener("click", (e) => {
      ...
      if (confirm("Are you sure you want to delete this note!")) {
        console.log("yes");
        window.location = `../KitchenLogin/KitchenshowOrder.php?delete=${lockid}`;
